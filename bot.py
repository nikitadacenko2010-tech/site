...
BOT_TOKEN = "8784995526:AAEP28cNfeDFi6Zk0ozD-ASLmt1iczo5HfA"
...
# Статистика воркеров {user_id: {'deals': 0, 'rating': 0.0}}
worker_stats = {}

# Реферальная система {user_id: {'referrals': 0, 'earned': 0.0, 'invited_by': user_id_or_none}}
referral_system = {}
...
def find_trustify_photo():
    ...
    search_locations = [current_path, downloads_path]
...
def get_currency_choice_kb():
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="⭐ Stars", callback_data="setup_cur_STAR"),
         InlineKeyboardButton(text="TON", callback_data="setup_cur_TON")],
        [InlineKeyboardButton(text="RUB", callback_data="setup_cur_RUB"),
         InlineKeyboardButton(text="UAH (UKR)", callback_data="setup_cur_UAH")],
        [InlineKeyboardButton(text="USDT", callback_data="setup_cur_USDT")],
        [InlineKeyboardButton(text="⬅ Назад", callback_data="ui_back_home")]
    ])
...
async def start_handler(message: Message):
    ...
    if len(args) > 1:
        ...
        if payload in active_deals:
            ...
            # ПРОВЕРКА: Если пользователь — создатель сделки
...
async def cb_manager(callback: CallbackQuery):
    ...
    if act == "ui_back_home":
        ...
        await send_interface_view(None, user_id, TEXT_REQUISITES_MANAGE, get_requisites_menu_kb(), True, callback)
        ...
        ref_text = TEXT_REFERRAL_MENU.format(
            stats['referrals'], 
            stats['earned'], 
            bot_info.username, 
            user_id
...
async def text_handler(message: Message):
    ...
    if state == "state_reqs":
        ...
        user_fsm_states[user_id] = "state_amt"
        await bot.edit_message_caption(chat_id=user_id, message_id=ui_cache[user_id], caption=TEXT_ASK_AMOUNT, reply_markup=get_back_to_main_kb(), parse_mode="Markdown")
    
    elif state == "state_amt":
        temp_data[user_id]["amount"] = message.text
        user_fsm_states[user_id] = "state_gift"
        await bot.edit_message_caption(chat_id=user_id, message_id=ui_cache[user_id], caption=TEXT_ASK_GIFT, reply_markup=get_back_to_main_kb(), parse_mode="Markdown")
        ...
        await bot.edit_message_caption(chat_id=user_id, message_id=ui_cache[user_id], caption=final, reply_markup=get_back_to_main_kb(), parse_mode="Markdown")
        ...
        await bot.edit_message_caption(chat_id=user_id, message_id=ui_cache[user_id], caption=thanks, reply_markup=get_back_to_main_kb(), parse_mode="Markdown")
